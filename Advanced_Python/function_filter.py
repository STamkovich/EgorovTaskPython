...
# Функция filter представляет собой итератор и в неё войдут только те элементы нашей итерабельной коллекции
# для которы[ функция вернёт значение True
# на функцию накладывается ограничение то есть она должна возвращать либо значение True либо False,
# то есть другие функции для работы с фильтром не подойдут
# а именно наша функция отфильтрует значения нашей коллекции и оставит только те которые вернуть значения True
...
b = list(filter(f, a))
b1 = [i for i in a if 9 < i < 100]  # итог тот же но при помощи генератора списка
print(b)  # [14, 79, 645, 7952, 18, 192, 471]
# так в функцию filter можно передавать встроенные функции
a1 = [14, 0, ' ', 5, [5], 79, 645, 'hello', 7952, '', 18, 0, [], 192, 471]
b2 = list(filter(bool, a1))
print(b2)  # [14, ' ', 5, [5], 79, 645, 'hello', 7952, 18, 192, 471]

a3 = ['world', 'hello', '23223', 'carrot', 'hi']
b3 = list(filter(lambda x: len(x) > 4, a3))
print(b3)  # ['world', 'hello', '23223', 'carrot']
# так же  внутрь функции фильтр вы можете в качестве функций передавать методы объектов
# но они должны возвращать True или False
a4 = '2442hjHGFlj4v2vhj'
b4 = list(filter(str.isdigit, a4))
print(b4)  # ['2', '4', '4', '2', '4', '2'
# как отфильтровать словарь
d = {
    'moscow': 800,
    'boston': 750,
    'LA': 400,
...
b5 = list(filter(lambda x: d[x] > 700, d))
print(b5)  # ['moscow', 'boston', 'SF']

# Задачки
# Напишите программу, которая отфильтрует список numbers так,
# чтобы в нем остались только четные значения. Используйте при этом lambda функцию.
...
a1 = list(filter(lambda x: x < 0, numbers))
a2 = list(filter(lambda x: x == 0, numbers))
a3 = list(filter(lambda x: x > 0, numbers))
